...
import { Team, Player } from "@wh-share/objects/team";
...
const defaultImg =
  "https://wp.willsbach-handball.de/wp-content/uploads/players/avatar_1522109382.png";
...
export class TeamService extends WillsbachApiService {
  ...
  getPlayer(playerId: string): Observable<Player> {
    return this.http.get<any>(`${this.url}players/${playerId}`).pipe(
      map((player) => {
        if (
          player.picture === "" ||
  ...
  getTeam(teamId: string): Observable<Team> {
    return this.http.get<Team>(this.getTeamUrl(teamId)).pipe(
      map((team) => {
        ...
        let picture;
        this.wp.getTeamPictures(false, team.wp.id).subscribe(
          (pic) => (picture = pic),
          (error) => {
            console.log(error);
          },
        ...
        if (team.coaches !== undefined) {
          ...
          team.coaches.forEach(function (part, index, coach) {
            if (
              coach[index].picture === undefined ||
              coach[index].picture === "" ||
              coach[index].picture === null
            ) {
  ...
  getTeamPictures(teamName: string): Observable<WPPicture[]> {
    ...
    return this.http.get<WPPicture[]>(`${apiTeams}?teamId=${teamName}`).pipe(
      map((team) => {
